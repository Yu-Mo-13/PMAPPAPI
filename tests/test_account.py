...
def test_create_account(db_session: AsyncSession):
    ...
    # Cleanup
    db_session.execute(select(account_model).filter(account_model.account == "new_account").filter(account_model.app == "new_app").delete())
    db_session.commit()
...
def test_delete_account(db_session: AsyncSession):
    ...
    # Verify deletion
    result = db_session.execute(select(account_model).filter(account_model.account == "delete_account").filter(account_model.app == "delete_app"))
