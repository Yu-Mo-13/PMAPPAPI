...
async def create_authority(name: str, adminflg: str, db: AsyncSession = Depends(get_db)):
    return await authority_controller.create_authority(db, name, adminflg, str(get_config("ISACTIVE")))
...
async def delete_authority(cd: int, db: AsyncSession = Depends(get_db)):
    return await authority_controller.delete_authority(db, cd, str(get_config("ISDELETE")))